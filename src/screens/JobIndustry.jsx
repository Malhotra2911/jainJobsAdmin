...
import { deleteJobIndustries, deleteJobIndustry,getJobIndustry} from '../redux/Action/CommonAction';
import AddJobIndustry from './InputDialog/AddJobIndustry';
...
export default function JobIndustry() {
    ...
    const [orderBy, setOrderBy] = React.useState('JI_NAME');
    ...
    const deleteBlogs = () => {
        if (selected.length == 1) {
            ...
            dispatch(deleteJobIndustry(jiId));
            window.location.reload(true);

        } else if (rows.length == selected.length) {
            dispatch(deleteJobIndustries());
            window.location.reload(true);
        }
    };
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = rows.map((n) => n.JI_ID);
    ...
    const handleClick = (event, JI_ID) => {
        const selectedIndex = selected.indexOf(JI_ID);
        let newSelected = [];

        if (selectedIndex === -1) {
            newSelected = newSelected.concat(selected, JI_ID);
    ...
    const isSelected = (JI_ID) => selected.indexOf(JI_ID) !== -1;
                ...
                <div className="table">
                 ...
                 rows.slice().sort(getComparator(order, orderBy)) */}
                                        {rows
                                            ...
                                            .map((row, index) => {
                                                const isItemSelected = isSelected(row.JI_ID);
                                                const labelId = `enhanced-table-checkbox-${index}`;

                                                return (
                                                    <TableRow
                                                        hover
                                                        onClick={(event) => handleClick(event, row.JI_ID)}
                                                        role="checkbox"
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell
                                                            ...
                                                            align="left">{row.JI_NAME}</TableCell>
                                                        <TableCell padding="checkbox"
