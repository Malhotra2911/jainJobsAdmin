...
export default function JobCategory() {
    ...
    const deleteBlogs = () => {
        if (selected.length == 1) {
            ...
            dispatch(deleteJobCategory(jcId));
            window.location.reload(true);

        } else if (rows.length == selected.length) {
            dispatch(deleteJobCategories());
            window.location.reload(true);
        }
    };
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = rows.map((n) => n.JC_ID);
    ...
    const handleClick = (event, JC_ID) => {
        const selectedIndex = selected.indexOf(JC_ID);
        let newSelected = [];

        if (selectedIndex === -1) {
            newSelected = newSelected.concat(selected, JC_ID);
    ...
    const isSelected = (JC_ID) => selected.indexOf(JC_ID) !== -1;
                ...
                <div className="table">
                 ...
                 rows.slice().sort(getComparator(order, orderBy)) */}
                                        {rows
                                            ...
                                            .map((row, index) => {
                                                const isItemSelected = isSelected(row.JC_ID);
                                                const labelId = `enhanced-table-checkbox-${index}`;

                                                return (
                                                    <TableRow
                                                        hover
                                                        onClick={(event) => handleClick(event, row.JC_ID)}
                                                        role="checkbox"
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell
                                                            ...
                                                            align="left">{row.JC_NAME}</TableCell>
                                                        <TableCell padding="checkbox"
