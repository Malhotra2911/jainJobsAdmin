...
import { deleteStory, getStory } from '../../redux/Action/JobseekerAction';
...
export default function StoryTable() {
    ...
    const [orderBy, setOrderBy] = React.useState('JC_USER_ID');
    ...
    const CityData = useSelector(
        (state) => state.JobSeekerReducer.story
    );
    // var array = CityData.map(function (a) { return a.YOUTUBE_LINK; });
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = rows.map((n) => n.STORY_ID);
    ...
    const handleClick = (event, STORY_ID) => {
        const selectedIndex = selected.indexOf(STORY_ID);
        let newSelected = [];

        if (selectedIndex === -1) {
            newSelected = newSelected.concat(selected, STORY_ID);
    ...
    const isSelected = (STORY_ID) => selected.indexOf(STORY_ID) !== -1;
                ...
                <Box sx={{ width: '100%' }}>
                 ...
                 rows.slice().sort(getComparator(order, orderBy)) */}
                                    {rows
                                        ...
                                        .map((row, index) => {
                                            const isItemSelected = isSelected(row.STORY_ID);
                                            const labelId = `enhanced-table-checkbox-${index}`;

                                            return (
                                                <TableRow
                                                    hover
                                                    onClick={(event) => handleClick(event, row.STORY_ID)}
                                                    role="checkbox"
                                                ...
                                                >
                                                    ...
                                                    <TableCell
                                                        ...
                                                        align="left">{row.USER_ID}
                                                    </TableCell>
                                                    <TableCell
                                                        sx={{ pl: 2, border: "3px solid black" }}
                                                        align="left"> <a href={row.YOUTUBE_LINK} target="_blank">{row.YOUTUBE_LINK}</a>
                                                    </TableCell>
                                                    <TableCell
                                                        sx={{ pl: 2, border: "3px solid black" }}
                                                        align="left">{row.DESCRIPTION}
                                                    </TableCell>
                                                    <TableCell padding="checkbox"
