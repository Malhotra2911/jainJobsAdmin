...
import { deleteSkill, deleteSkills, getQuestions, getSkills } from '../../redux/Action/CommonAction';
...
const TempQuestions = () => {
    ...
    const SkillsData = useSelector(
        (state) => state.CommonReducer.questions
    );
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = rows.map((n) => n.QUESITION_ID);
    ...
    const handleClick = (event, QUESITION_ID) => {
        const selectedIndex = selected.indexOf(QUESITION_ID);
        let newSelected = [];

        if (selectedIndex === -1) {
            newSelected = newSelected.concat(selected, QUESITION_ID);
    ...
    const isSelected = (QUESITION_ID) => selected.indexOf(QUESITION_ID) !== -1;
                ...
                <div className="admin_dashboard_container">
                 ...
                 rows.slice().sort(getComparator(order, orderBy)) */}
                                            {rows
                                                ...
                                                .map((row, index) => {
                                                    const isItemSelected = isSelected(row.QUESITION_ID);
                                                    const labelId = `enhanced-table-checkbox-${index}`;

                                                    return (
                                                        <TableRow
                                                            hover
                                                            onClick={(event) => handleClick(event, row.QUESITION_ID)}
                                                            role="checkbox"
                                                            aria-checked={isItemSelected}
                                                            tabIndex={-1}
                                                            sx={{ background: "#EEEEEE" }}
                                                            key={row.QUESITION_ID}
                                                        ...
                                                        >
                                                            ...
                                                            <TableCell
                                                                ...
                                                                align="left">{row.QUESITION_TITLE}
                                                            </TableCell>
                                                            <TableCell padding="checkbox"
