...
import {deleteContacts, getContacts} from '../../redux/Action/CommonAction';
...
export default function JobseekerContacts() {
    const [order, setOrder] = React.useState('asc');
    const [orderBy, setOrderBy] = React.useState('JC_NAME');
    ...
    const JobSeekerContactData = useSelector(
        (state) => state.CommonReducer.contacts
    );
    console.log(JobSeekerContactData);
    ...
    // console.log(JOBSeekerFeedbackData);

    const rows = JobSeekerContactData;
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = rows.map((n) => n.CONTACT_ID);
    ...
    const handleClick = (event, CONTACT_ID) => {
        const selectedIndex = selected.indexOf(CONTACT_ID);
        let newSelected = [];

        if (selectedIndex === -1) {
            newSelected = newSelected.concat(selected, CONTACT_ID);
    ...
    const isSelected = (CONTACT_ID) => selected.indexOf(CONTACT_ID) !== -1;
                                    ...
                                    {rows
                                        ...
                                        .map((row, index) => {
                                            ...
                                            return (
                                                <TableRow
                                                    hover
                                                    onClick={(event) => handleClick(event, row.CONTACT_ID)}
                                                    role="checkbox"
